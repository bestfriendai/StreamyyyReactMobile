...
export const MultiStreamPerformanceOptimizer: React.FC<PerformanceOptimizerProps> = ({
  ...
  // Performance monitoring interval (disabled for performance)
  useEffect(() => {
    if (!enableAutoOptimization) {
      return;
    }

    // Only run performance checks if explicitly enabled and we have many streams
  ...
  // Disabled initial performance analysis for faster loading
  // useEffect(() => {
  ...
  const handleAutoOptimization = useCallback(
    async (report: StreamPerformanceReport) => {
      const now = Date.now();
      ...
      if (timeSinceLastOptimization < 120000) {
        return;
      }
      ...
      if (report.totalMemoryUsage > 300) {
        // More than 300MB
  ...
  const optimizeStreamCount = useCallback(async () => {
    ...
    if (streamsToRemove > 0) {
      ...
      const streamHealths = activeStreams
        .map(stream => ({
          stream,
