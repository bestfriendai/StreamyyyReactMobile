...
import { EnhancedStreamCardV2 } from '@/components/EnhancedStreamCardV2';
...
export const EnhancedDiscoverScreenV4: React.FC<EnhancedDiscoverScreenV4Props> = ({
  ...
  const streamSections: StreamSection[] = useMemo(() => {
    ...
    if (processedStreams.length > 0) {
      ...
      const trendingStreams = processedStreams
        .filter(stream => (stream as any).isTrending || (stream as any).isHot || stream.viewer_count > 2000)
      ...
      if (selectedCategory !== 'all') {
        ...
        if (categoryStreams.length > 0) {
          const categoryName = GAME_CATEGORIES.find(cat => cat.id === selectedCategory)?.name || 'Category';
          sections.push({
            id: 'category',
        ...
        <View style={styles.headerMain}>
            ...
            <Animated.View style={[styles.statsContainer, statsAnimatedStyle]}>
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statText}>
                  {processedStreams.reduce((acc, stream) => acc + stream.viewer_count, 0).toLocaleString()} viewers
                </Text>
              </View>
            </Animated.View>
          </View>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity style={styles.headerButton} onPress={toggleSearch}>
              <LinearGradient
                colors={showSearch ? ['#4B5563', '#6B7280'] : ['rgba(255,255,255,0.1)', 'rgba(255,255,255,0.05)']}
            ...
            <TouchableOpacity style={styles.headerButton} onPress={toggleFilters}>
              <LinearGradient
                colors={showFilters ? ['#4B5563', '#6B7280'] : ['rgba(255,255,255,0.1)', 'rgba(255,255,255,0.05)']}
                style={styles.headerButtonGradient}
              >
  ...
  const renderStreamGrid = (streams: TwitchStream[], sectionId: string, showAll: boolean = false) => {
