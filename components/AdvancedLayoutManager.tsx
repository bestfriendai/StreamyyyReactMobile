...
export type LayoutType = 
  | 'grid_1x1' | 'grid_2x2' | 'grid_3x3' | 'grid_4x4'
  | 'linear_2x1' | 'linear_3x1' | 'linear_4x1'
  | 'pip' | 'focus' | 'stacked' | 'mosaic'
...
export function AdvancedLayoutManager({
  ...
  const basicLayouts = useMemo(() => [
    ...
    { id: 'pip', name: 'Picture-in-Picture', icon: PictureInPicture, description: 'Main + thumbnails' },
  ...
  const advancedLayouts = useMemo(() => [
    ...
    { id: 'split_horizontal', name: 'Horizontal Split', icon: Split, description: 'Top/bottom split' },
    { id: 'split_vertical', name: 'Vertical Split', icon: Split, description: 'Left/right split' },
  ...
  const getOptimalLayoutForStreamCount = (count: number): LayoutType => {
    if (count === 1) return 'grid_1x1';
    if (count === 2) return 'split_horizontal';
    if (count <= 4) return 'grid_2x2';
    if (count <= 6) return 'linear_3x1';
    if (count <= 9) return 'grid_3x3';
    return 'mosaic';
  };
  ...
  const renderLayoutOption = (layout: any) => {
    ...
    return (
      <AnimatedTouchableOpacity
        ...
        style={[
          styles.layoutOption,
          isActive && styles.activeLayoutOption,
        ]}
        ...
        >
          ...
          <View style={styles.layoutInfo}>
            <Text
              style={[
                styles.layoutName,
                isActive && styles.activeLayoutName,
              ]}
            >
              {layout.name}
            </Text>
            <Text
              style={[
                ...
                isActive && styles.activeLayoutDescription,
              ]}
            >
              {layout.description}
            </Text>
